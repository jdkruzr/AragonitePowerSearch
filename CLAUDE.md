...
- **HWR prewarm held for Activity lifetime.** `MainActivity` binds `com.onyx.android.ksync/.service.KHwrService` in `onCreate()` via its own `ServiceConnection` (separate from `AragoniteHWR`'s singleton) and unbinds in `onDestroy()`. Purpose: keep the ksync process warm so `IndexingService`'s subsequent `bindAndAwait()` completes quickly instead of hitting cold-start latency. The prewarm `ServiceConnection` is independent, so `Indexer`'s finally-block `hwr.unbind()` after an indexing run does NOT tear down the prewarm binding.
- **`AragoniteHWR.bindAndAwait` default timeout is 8 seconds.** Onyx Boox cold-start of ksync has been observed at multiple seconds; the earlier 2-second default caused false "HWR unavailable" errors on fresh launches. Combined with the MainActivity prewarm, bind usually completes in milliseconds, but the extended timeout is belt-and-suspenders for the case where prewarm failed or the process was killed by the OS.
